...
DROP TABLE IF EXISTS `%{schema}`.`raws`;
DROP TABLE IF EXISTS `%{schema}`.`cells`;
DROP TABLE IF EXISTS `%{schema}`.`rows`;
DROP TABLE IF EXISTS `%{schema}`.`columns`;
DROP TABLE IF EXISTS `%{schema}`.`tables`;
DROP TABLE IF EXISTS `%{schema}`.`log_changements`;
...
CREATE TABLE `%{schema}`.`log_changements` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
